...
Un context manager est un objet qu'on appelle avec `with`. Il va définir deux méthodes, `__enter__`et `__exit` pour décrire le code qui doit être exécuté avant et après le bloc dans le `with`
