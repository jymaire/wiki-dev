...
## Indexation
  ...
  Pour cela, il va donc falloir ingérer des données. Une fois que l’on a converti les données en format texte, on va pouvoir les découper en petits bouts (tokens). L’opération est appelée chunking. En général, il y a un chevauchement entre les tokens pour être sûr de ne pas manquer d'informations. Maintenant que nous avons des extraits de phrases, il va falloir les convertir dans un format que comprend une machine : c’est l’embedding. Chaque token (suite de mots) va être converti en un vecteur à N dimensions. Donc pour chaque suite de mots, nous allons pouvoir y associer une coordonnée. La distance entre deux vecteurs sera la distance sémantique entre deux tokens. Ces vecteurs sont stockés dans une base de données dite “vectorielle”. À partir d’une entrée utilisateur, on va pouvoir extraire les tokens qui sont proches en sens.
